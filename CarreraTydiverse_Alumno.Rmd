...
```{r}
subset(cran, package == "plotly" & r_version == "3.5.1")
```
...
```{r}
subset(cran, country == "ES" & r_version > "3.4.0")
```
...
subset(cran, country == "ES" | country == "PT")
...
```{r}
subset(cran, size > 100500 & r_os == "linux-gnu")
...
```{r}
cran2[order(cran2$ip_id),]
```

12. Ordena `cran2` en orden descendente según la variable `ip_id`.

```{r}
cran2[order(cran2$ip_id, decreasing = TRUE),]
...
```{r}
cran2[order(cran2$package, cran2$ip_id),]
```
...
```{r}
cran2[order(cran2$country, cran2$r_version, cran2$ip_id), decreasing = c(F, T, F), ]
```
...
```{r}
avg_bytes<-cran %>% summarise(avg_bytes=mean(size))
avg_bytes
...
```{r}
by_package %>% summarise(mean(size))
...
```{r}
pack_sum <- by_package %>% summarise(count = n(), unique = n_distinct(ip_id),
                         countries = n_distinct(country), med_bytes = median(size))
...
```{r}
top_counts <- pack_sum %>%count(package)%>%
